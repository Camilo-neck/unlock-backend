...
class EventController:
    # Dependencies
    @staticmethod
    def verify_event_existence(event_id: uuid.UUID, admin_id: uuid.UUID) -> Event:
    ...
    @staticmethod
    def get_participant_events(user_id: uuid.UUID) -> List[Event]:
    ...
    @staticmethod
    def get_event_by_id(event_id: uuid.UUID) -> Event:
        event = events_table.select("*").eq("id", event_id).execute()
        return event.data[0] if event.data else None

    @staticmethod
    def event_exists(event_id: uuid.UUID, admin_id: uuid.UUID) -> bool:
        return True if EventController.get_event_by_id(event_id) else False
    
    @staticmethod
    def update_event_capacity(event_id: uuid.UUID, capacity: int) -> Event:
